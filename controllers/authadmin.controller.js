...
exports.register = async (req, res) => {
  ...
  return res.status(201).json({ success: true, message: "Akun berhasil dibuat silahkan login" });
};
...
exports.login = async (req, res) => {
  ...
  const token = jwt.sign({ id: user.id, email: user.email , username: user.username }, "Rossycake_key_admin", {
    expiresIn: "1d",
  });
